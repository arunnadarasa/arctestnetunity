...
//        } 
    ...
    public async void setOptions(Web3AuthOptions web3AuthOptions)
    {
        ...
        if (!isfetchConfigSuccess)
        {
            ...
            if (this.web3AuthOptions.whiteLabel != null)
                this.initParams["whiteLabel"] = JObject.FromObject(this.web3AuthOptions.whiteLabel, JsonSerializer.Create(settings));

            // v10 hosted auth: these MUST be JSON arrays (never omit — UI error "authConnectionConfig should be an array").
            if (this.web3AuthOptions.loginConfig != null)
            {
                var loginConfigArr = new JArray();
                foreach (var kv in this.web3AuthOptions.loginConfig)
                {
                    if (kv.Value != null)
                        loginConfigArr.Add(JObject.FromObject(kv.Value, JsonSerializer.Create(settings)));
                }
                this.initParams["loginConfig"] = loginConfigArr;
                this.initParams["authConnectionConfig"] = loginConfigArr;
            }
            else
            {
                var emptyConnections = new JArray();
                this.initParams["loginConfig"] = emptyConnections;
                this.initParams["authConnectionConfig"] = emptyConnections;
            }
            ...
            if (this.web3AuthOptions.chainNamespace != null)
                this.initParams["chainNamespace"] = this.web3AuthOptions.chainNamespace.ToString().ToLower();

            if (this.web3AuthOptions.mfaSettings != null)
                this.initParams["mfaSettings"] = JObject.FromObject(this.web3AuthOptions.mfaSettings, JsonSerializer.Create(settings));
            ...
            if (this.web3AuthOptions.chainConfig != null)
                this.initParams["chainConfig"] = JObject.FromObject(this.web3AuthOptions.chainConfig, JsonSerializer.Create(settings));

            // After initParams (including redirectUrl) is merged — authorizeSession needs correct Origin for v2 store/get.
            authorizeSession("");
    ...
    /// <summary>Becomes true after setOptions has fully completed, including fetchProjectConfig.</summary>
    public bool isOptionsSet { get; private set; } = false;
    ...
    private async void processRequest(string path, LoginParams loginParams = null)
    {
        ...
        PatchV10AuthConnectionIntoInitParams();
        ...
        Debug.Log("[W3A] paramMap: =>" + JsonConvert.SerializeObject(paramMap, Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore }));
        ...
        if (!string.IsNullOrEmpty(loginId))
        {
            ...
            {
                uriBuilder.Path = uriBuilder.Path.TrimEnd('/') + "/start";
            }
            uriBuilder.Fragment = "b64Params=" + hash;
            Debug.Log("[W3A] finalUriBuilderToOpen: =>" + uriBuilder.ToString());
    ...
    public async void launchWalletServices(ChainConfig chainConfig, string path = "wallet")
    {
            ...
            Debug.Log("[W3A] paramMap: =>" + JsonConvert.SerializeObject(paramMap));
            string loginId = await createSession(JsonConvert.SerializeObject(paramMap, Formatting.None,
                new JsonSerializerSettings
                {
                    NullValueHandling = NullValueHandling.Ignore
    ...
    /// <summary>
    /// v10 hosted auth requires options.loginConfig and options.authConnectionConfig as JSON arrays on every login session.
    /// Merges from Web3AuthOptions.loginConfig when set; otherwise injects default Google (w3a-google unless options.defaultGoogleAuthConnectionId is set).
    /// </summary>
    private void PatchV10AuthConnectionIntoInitParams()
    {
        JsonSerializerSettings settings = new JsonSerializerSettings
        {
            Converters = new List<JsonConverter> { new StringEnumConverter() },
        ...
        JArray arr = null;
        if (web3AuthOptions?.loginConfig != null && web3AuthOptions.loginConfig.Count > 0)
        {
            arr = new JArray();
            foreach (var kv in web3AuthOptions.loginConfig)
            {
                if (kv.Value != null)
                    arr.Add(JObject.FromObject(kv.Value, JsonSerializer.Create(settings)));
            }
        }
        if (arr == null || arr.Count == 0)
        {
            // Must match a Google auth connection enabled for this client id (often w3a-google on Sapphire; doc samples may use w3a-google-demo).
            var connId = !string.IsNullOrEmpty(web3AuthOptions?.defaultGoogleAuthConnectionId)
                ? web3AuthOptions.defaultGoogleAuthConnectionId
                : "w3a-google";
            var jo = new JObject
            {
                ["verifier"] = connId,
        ...
        this.initParams["loginConfig"] = arr;
        this.initParams["authConnectionConfig"] = arr;
    }

    private string GetCurrentRedirectUrlForSessionStore()
    {
        if (initParams != null && initParams.ContainsKey("redirectUrl") && initParams["redirectUrl"] != null)
            return initParams["redirectUrl"].ToString();
    ...
    private void authorizeSession(string newSessionId)
    {
        ...
        if (!string.IsNullOrEmpty(sessionId))
        {
            ...
            var redirectForOrigin = GetCurrentRedirectUrlForSessionStore();
            StartCoroutine(Web3AuthApi.getInstance().authorizeSession(pubKey, redirectForOrigin, (response =>
            {
    ...
    private void sessionTimeOutAPI()
    {
        ...
        if (!string.IsNullOrEmpty(sessionId))
        {
            ...
            StartCoroutine(Web3AuthApi.getInstance().authorizeSession(pubKey, redirectForOrigin, (response =>
            {
                ...
                var logoutAllowedOrigin = "*";
                var logoutRu = GetCurrentRedirectUrlForSessionStore();
                if (!string.IsNullOrEmpty(logoutRu) && Uri.TryCreate(logoutRu, UriKind.Absolute, out var logoutOriginUri))
                    logoutAllowedOrigin = logoutOriginUri.GetLeftPart(UriPartial.Authority);
                ...
                StartCoroutine(Web3AuthApi.getInstance().logout(
                        new LogoutApiRequest()
                        {
                            ...
                            timeout = 1,
                            allowedOrigin = logoutAllowedOrigin
                        }, result =>
                        {
                            if (result != null)
                            {
    ...
    private async Task<string> createSession(string data, long sessionTime)
    {
        ...
        var jsonData = JsonConvert.SerializeObject(encryptedMetadata);
        var allowedOrigin = "*";
        var ru = GetCurrentRedirectUrlForSessionStore();
        if (!string.IsNullOrEmpty(ru) && Uri.TryCreate(ru, UriKind.Absolute, out var originUri))
            allowedOrigin = originUri.GetLeftPart(UriPartial.Authority);
    ...
    private async Task<bool> fetchProjectConfig()
    {
        ...
        StartCoroutine(Web3AuthApi.getInstance().fetchProjectConfig(this.web3AuthOptions.clientId, this.web3AuthOptions.network.ToString().ToLower(), (response =>
        {
            if (response != null)
            {
                ...
                Debug.Log("[W3A] fetchProjectConfig originData: =>" + JsonConvert.SerializeObject(this.web3AuthOptions.originData));
                ...
                if(this.web3AuthOptions.originData != null)
                    this.initParams["originData"] = JObject.FromObject(this.web3AuthOptions.originData);
